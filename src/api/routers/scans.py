...
from ...database import get_scan, get_scans, create_scan_placeholder, update_scan, db_context, get_db, Scan  # type: ignore
from ..schemas import ScanSummary, ScanDetails
from ...engine import scan_network, convert_scan_to_schema, save_device_results_to_db  # type: ignore

router = APIRouter(prefix="/scans", tags=["scans"])


@router.get("/", response_model=List[ScanSummary])
def list_scans(skip: int = 0, limit: int = 10, db: Session = Depends(get_db)) -> List[Scan]:
    return get_scans(db, skip=skip, limit=limit)


@router.get("/{scan_id}", response_model=ScanDetails)
def get_scan_details(scan_id: int, db: Session = Depends(get_db)) -> Scan:
...
def post_scan(background_tasks: BackgroundTasks, db: Session = Depends(get_db)) -> Scan:
